...
void onPreUpdate() {
    ...
    if (status == 3 && client.getScreen().equals("GuiChest") && inventory.getChest().endsWith("Chest")) {
        ...
        if ((automatic && !foundItemsInChest && !foundAllItemsInInventory) || (keybinds.isKeyDown(keybinds.getKeyCode(keyNames[(int) modules.getSlider(scriptName, "Inventory To Chest")])) && delayTicks == 0)) {
        ...
        if ((automatic && !foundItemsInInventory && !foundAllItemsInChest) || (keybinds.isKeyDown(keybinds.getKeyCode(keyNames[(int) modules.getSlider(scriptName, "Chest To Inventory")])) && delayTicks == 0)) {
